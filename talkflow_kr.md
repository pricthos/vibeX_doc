...
// 4단계: 방 진입은 enterRoom 한 번으로 (권장)
const detail = await client.enterRoom('room-id');
// ↑ 내부에서 subscribe → setActive → getRoom 을 race-free 순서로 자동 수행
// 다른 방으로 이동: await client.enterRoom('other-room-id')
// 리스트 화면으로 이동: client.clearActiveRoom()
```
...
const detail = await client.enterRoom(roomId); // 방 입장 (subscribe + setActive + getRoom 자동)
...
client.on('callStarted', handler);        // 통화 시작
```
...
// 채팅방 입장 — 권장 진입 경로 (subscribe → setActive → getRoom 을 race-free 순서로 자동 수행)
const roomDetail = await client.enterRoom('room-id');
// { room: { id, roomName, ... }, messages: [...] }
//
// ⚠️ getRoom() 만 단독 호출은 권장하지 않습니다.
//   subscribe 가 getRoom 보다 나중에 실행되면 서버의 "방 입장 시 read 이벤트" 가 유실되어
//   상대방 화면의 unread 표시("1")가 영구 고착될 수 있습니다.
//   세밀한 제어가 필요한 경우에만 subscribeRoom / setActiveRoom / getRoom 을 수동 호출하세요.

// 방 정보만 필요할 때 (입장 처리 없이 — 미리보기/권한 체크용)
...
채팅방에서 실시간 메시지를 받으려면 구독이 필요합니다. **일반적인 경우 `enterRoom()` 하나로 충분**하며, 세밀한 제어가 필요할 때만 아래 메서드를 개별 호출하세요.

**기본 흐름 (`enterRoom` 사용 — 권장)**:
1. 방 입장 — `enterRoom(roomId)` — subscribe + setActive + getRoom 자동 수행
2. 다른 화면 이동 시 — `clearActiveRoom()` — 읽음 처리 중단
3. 앱 종료/화면 이탈 시 — `unsubscribeRoom()` — 구독 해제

**수동 흐름 (세밀한 제어가 필요할 때만)**:
...
> **멱등성 설계**: SDK가 요청마다 UUID를 자동 생성하여 서버에 전송합니다. 호출자는 `messageId`를 지정하지 **않습니다**. 같은 UUID의 재시도는 서버에서 기존 결과를 그대로 반환합니다 (`duplicate: true`).
>
> **파일 분리 (`separateFiles: true`)**: 한 요청이 N개의 메시지로 저장될 수 있습니다. 각 메시지는 **서버가 부여한 고유 `messageId`**를 가지며, 응답의 `messages` 배열에 담겨 돌아옵니다. 답글/삭제/리액션은 이 서버 발급 `messageId`로 수행해야 합니다.
...
| `replyToMessageId` | `string` | 선택 | 답글 대상 원본 메시지의 서버 `messageId` |
...
```javascript
{
    duplicate: false,        // 재시도로 인한 멱등 히트면 true
    messages: [
        {
            messageId: 'srv-uuid-1', // 서버가 부여한 고유 ID — 답글/삭제/리액션에 사용
            roomId: '...',
            userId: '...',
            content: '...',
...
```javascript
// 간편 텍스트 전송
const { messages } = await client.sendTextMessage('room-id', '안녕하세요!');
console.log(messages[0].messageId);   // 서버 발급 ID

// 상세 메시지 전송 — messageId는 SDK가 내부에서 생성하므로 전달 금지
const result = await client.sendMessage('room-id', {
    message: '안녕하세요!'
});
if (result.duplicate) {
    console.log('이미 처리된 요청 — 기존 결과 반환');
}
...
// 답글 전송 — 원본 메시지의 서버 messageId를 지정하면 서버가 snapshot을 자동으로 첨부합니다
await client.sendReply('room-id', '저도 동의해요!', 'original-srv-msg-id');
```
...
한 사용자는 메시지당 하나의 이모지만 보유합니다. 같은 이모지 재전송 시 해제, 다른 이모지 전송 시 교체됩니다.

```javascript
// 리액션 토글 — 같은 이모지 재호출 시 해제, 다른 이모지 호출 시 교체
...
- SDK는 `POST /api/v1/link-preview`를 사용해 URL을 body로 전송합니다.
- 미리보기를 만들 수 없으면 `null`을 반환합니다.
- 서명 URL, 토큰 포함 URL 같은 민감한 링크가 query string으로 남지 않도록 설계되었습니다.
...
| `sendMessage` | `separateFiles` | 기본 `true`. `false`면 동일 타입 그룹핑 (`GALLERY`) |
| `sendMessage` | `replyToMessageId` | 답글 대상 메시지의 서버 `messageId` (같은 방 내) |
| `sendMessage` | (반환) | `{ duplicate, messages[] }` — `messages[i].messageId`가 서버 발급 고유 ID |
...
| API | 필드 | 제약 |
|---|---|---|
| `toggleReaction` | `emoji` | 이모지 문자 (예: `'👍'`, `'😂'`). 같은 이모지 재호출 시 해제, 다른 이모지 시 교체 |
